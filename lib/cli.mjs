...
export function validateQueryOptions(options) {
  ...
  if ((options.command === 'list' || options.command === 'search' || options.command === 'projects') && (options.comments || options.commentsOnly || options.fieldsRequested || options.fieldNames.length > 0 || options.linkedIssues || options.spentTime || options.workItems)) {
    console.error('--comments, --comments-only, --fields, --field, --linked-issues, --spent-time, and --work-items are only supported for single-issue lookup.');
    process.exit(1);
  }
  ...
  if ((options.comments || options.commentsOnly) && (options.fieldsRequested || options.fieldNames.length > 0 || options.linkedIssues || options.spentTime || options.workItems)) {
    console.error('Use only one of --comments, --comments-only, --fields/--field, --linked-issues, --spent-time, or --work-items.');
    process.exit(1);
  }

  if ((options.fieldsRequested || options.fieldNames.length > 0) && (options.linkedIssues || options.spentTime || options.workItems)) {
    console.error('Use only one of --fields/--field, --linked-issues, --spent-time, or --work-items.');
    process.exit(1);
  }
