...
function findFirstNonOptionToken(argv) {
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--profile' || arg === '--alias' || arg === '-c' || arg === '--config' || arg === '-s' || arg === '--search' || arg === '-n' || arg === '--limit' || arg === '--base-url') {
      index += 1;
      continue;
    }
...
export function parseArgs(argv) {
  ...
  if ((options.listRequested || options.searchQuery || options.projects) && (options.attachments || options.downloadAttachment)) {
    console.error('--attachments and --download-attachment are only supported for single-issue lookup.');
    process.exit(1);
  }
...
export function validateQueryOptions(options) {
  ...
  if (options.attachments && options.downloadAttachment) {
    console.error('Use only one of --attachments or --download-attachment.');
    process.exit(1);
  }
